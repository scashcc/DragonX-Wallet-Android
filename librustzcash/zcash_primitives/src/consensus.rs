...
impl Parameters for MainNetwork {
    fn activation_height(&self, nu: NetworkUpgrade) -> Option<BlockHeight> {
        match nu {
            // DragonX: Overwinter and Sapling are active from height 1 (the chain
            // activated them at/near genesis, per the dragonxd full node). DragonX has
            // NO network upgrades after Sapling, so Blossom/Heartwood/Canopy/Nu5 are
            // disabled (None). This keeps the consensus branch id at Sapling
            // (0x76b809bb) for every block height, exactly matching the node.
