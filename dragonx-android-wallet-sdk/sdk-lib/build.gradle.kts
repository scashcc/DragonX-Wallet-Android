...
mavenPublishing {
    ...
    // Only sign when explicitly enabled (needs GPG keys). For local/CI publishToMavenLocal
    // builds, RELEASE_SIGNING_ENABLED=false so we skip signing entirely.
    if (project.property("RELEASE_SIGNING_ENABLED").toString().toBoolean()) {
        signAllPublications()
    }
